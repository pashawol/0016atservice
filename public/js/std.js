...
function AddToBasket(ids, fx) {
	...
	options = {
		to: ".basket",
		className: "ui-transfer_gray"
	};
	$(fx).effect('transfer', options, 1000, function () {
		$('.basket').load('admin/basket.php?act=add_goods&id=' + ids);
...
function AddToCompare(ids, fx) {
	...
	$(fx).effect('transfer', options, 1000, function () {
		$('#comp').load('admin/basket.php?act=add_compare&id=' + ids);
	});
}
...
function AddToBasketLst(ids, fx) {
	...
	$(fx).effect('transfer', options, 1000, function () {
		$('.basket').load('admin/basket.php?act=add_goods&id=' + ids + '&vname=' + variant);
	});
}
...
function LoadBasket() {
	...
	$('._summary').load('admin/basket.php?act=get_summary'); //$('.basket_bg').fadeIn('slow');
}
...
function ShowBSForm() {
	...
	$('.changevar').each(function () {
		...
		if ($ob.val() == 'na') {
			sbsc = false; //$($ob).parent().addClass('restrict');

			$($ob).parent().effect('highlight', 1500, function () {});
		}
	});
	if (!sbsc) return false;
	$('.basket_sent').hide();
	$('.basket_summary').show();
...
function DeliveryOrder() {
	console.log("Delivering order..."); //$('.basket_data').load('admin/basket.php?act=deliveryOrder');
	...
	if (phone.length < 7) {
		...
		$('#phone').effect('pulsate', options, 1500, function () {
			$("#phone").removeClass('restrict');
		});
	...
	if (email.length < 5) {
		...
		$('#email').effect('pulsate', options, 1500, function () {
			$("#email").removeClass('restrict');
		});
	...
	$('.basket_form').slideUp('slow');
	$('._summary').hide();
	$.post("admin/basket.php?act=deliveryOrder", {
		firstname: firstname,
...
function VarSwitch() {
	...
	$('.varswitchlist a').click(function () {
		...
		$(this).parent().parent().find('.price_n').html($(this).find('span').html() + ' <span>���.</span>');
		$(this).html(OldLnk);
