...
$(document).ready(function () {
	...
	$('#_subs').click(function () {
		...
		$.post("admin/ddx.php?act=sendForm", {
			capt: capt,
			details: details,
			name: name,
			phone: phone,
	...
	$('#_lgn').click(function () {
		...
		$.post("/admin/ddx.php?act=login", {
			login: login,
			pass: pass
		}, function (data) {
			if (data == 'OK') {
				window.document.location = '/my_profile';
			} else {
				$('#wrong').fadeIn('slow');
			}
		});
	...
	$('#fb_add').click(function () {
		...
		var parent = $('#fb_parent').val(); ///console.log(rating);
		...
		$('#done').show();
		$.post('/admin/ddx.php?act=addcomment', {
			name: name,
			city: city,
			comment: comment,
			rating: rating,
	...
	$('.mmholder>a').hover(function () {
		...
		var rel = $(this).attr('rel'); //$('#'+rel).slideUp('fast');

		if (!$('#' + rel).length) {
			$('.mmholder>a').removeClass('act');
		}
	});
	$('.ddnmenu').hover(function () {}, function () {
		$(this).slideUp('fast');
		$('.mmholder>a').removeClass('act');
	});
...
function Log(txt, mode) {
	event_id++;
	if (!mode) console.log(event_id + '>>' + txt);else {
		switch (mode) {
			case "info":
...
function arrange() {
	wx = $(window).width();
	wy = $(window).height(); //console.clear();
}
